...
urlpatterns = [
    ...
    re_path(r"^(?P<model_str>category|product|sale|purchase|expense|expensecategory|otherincome|otherincomecategory|customer)$",
            views.generic_list_view, name="list"),
    # CRUD genérico para modelos simples (excepto product, que usa su propia vista)
    re_path(r"^(?P<model_str>category|expense|expensecategory|otherincome|otherincomecategory|customer)/new$",
            views.generic_form_view, name="new"),
    re_path(r"^(?P<model_str>category|expense|expensecategory|otherincome|otherincomecategory|customer)/(?P<pk>\d+)/edit$",
